...
module.exports = {
   ...
   getRecipe: (search) => {
      return new Promise((resolve, reject) => {
         conn.query(
            `SELECT recipe.id_recipe, recipe.name_recipe, recipe.ingredients, recipe.images, recipe.video, users.id_users, users.name, recipe.create_at
      FROM recipe INNER JOIN users ON users.id_users = recipe.id_users WHERE name_recipe LIKE '%${search}%' ORDER BY create_at ASC`,
            (err, res) => {
               if (err) {
                  reject(new Error(`SQL : ${err.message}`));
               } else {
   ...
   getRecipeByName: (name) => {
      return new Promise((resolve, reject) => {
         conn.query(
            `SELECT recipe.id_recipe, recipe.name_recipe, recipe.ingredients, recipe.images, recipe.video, users.id_users, users.name, recipe.create_at
      FROM recipe INNER JOIN users ON users.id_users = recipe.id_users WHERE name_recipe = $1`,
            [name],
            (err, res) => {
               if (err) {
                  reject(new Error(`SQL : ${err.message}`));
